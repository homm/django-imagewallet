...
def crop(width=None, height=None, halign='50%', valign='50%', background=None):
    ...
    них может быть не укзан, такой рахмер не будет изменен. Пераметры halign
    и valign задают расположение исходного изображения в новом. Если размеры
    увеличиваются и режим изображения не поддерживает прозрачность, пустое
    место заливается цветом background. Если этот цвет не задан, берется цвет
    background у формата.
    """
    # Парсим параметры.
    width_handler = size_handler(width)
    height_handler = size_handler(height)
