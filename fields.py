...
class WalletField(FileField):
    ...
    def __init__(self, verbose_name=None, name=None, upload_to='',
        ...
        Upload_to — строка или функция, которая должна возвращать полное имя
            файла с заменой для расширения {e}. Вызывается с тремя аргументами:
            поле, объект модели, в которую будет сохранен файл и имя
            оригинального файла. Если строка, то обозначает только директоррию.
            Все вхождения строки '%r' заменяются на случайный символ, дальше
            строка передается в метод strftime текущего времени.
